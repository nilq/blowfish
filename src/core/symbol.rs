...
use core::ast::{DisplayEnv, IdentEnv};
use core::fnv::FnvMap;
...
impl Deref for Symbol {
    ...
    fn deref(&self) -> &SymbolRef{
        ...
        unsafe {
            ::std::mem::transmute::<&str, &SymbolRef>(s)
        }
    }
}
...
impl Name {
    ...
    pub fn new<N: ?Sized + AsRef<str>>(n: &N) -> &Name {
        unsafe {
            ::std::mem::transmute::<&str, &Name>(n.as_ref())
        }
    }
...
impl<'a> SymbolModule<'a> {
    pub fn new(module: String, symbols: &'a mut Symbols) -> SymbolModule {
        SymbolModule {
            symbols: symbols,
